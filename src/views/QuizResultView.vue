<template>
  <div class="min-h-[calc(100vh-80px)] py-6 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
    ...
    <!-- Result Card -->
    <div class="bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-slate-200 text-center relative overflow-hidden mb-8">
      
      <!-- Background subtle soft glow -->
      <div class="absolute -top-12 left-1/2 -translate-x-1/2 w-96 h-96 bg-sky-100/50 rounded-full blur-3xl pointer-events-none"></div>

      <!-- Category badge -->
      <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-slate-700 font-fredoka text-xs mb-4">
        <span>{{ result.categoryIcon || '🎯' }} {{ result.categoryName || 'Test de Inglés' }}</span>
      </div>
      ...
      <h2 class="text-3xl sm:text-5xl font-bold font-fredoka text-slate-800 tracking-tight mb-2">
        {{ getTitleForScore(result.score, result.total) }}
      </h2>

      <p class="text-slate-500 text-base sm:text-lg max-w-md mx-auto mb-8 font-medium">
        {{ getSubtitleForScore(result.score, result.total) }}
      </p>

      <!-- Score Gauge Display -->
      <div class="inline-flex items-center justify-center p-6 rounded-3xl bg-sky-50 border border-sky-100 shadow-sm mb-8 min-w-[240px]">
        <div class="text-center">
          <span class="text-5xl sm:text-6xl font-bold font-fredoka text-sky-600">
            {{ result.score }} / {{ result.total }}
          </span>
          <span class="block text-xs font-bold text-sky-800 uppercase tracking-widest mt-1">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
        ...
        <router-link 
          ...
          class="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-fredoka font-bold text-base transition flex items-center justify-center gap-2 shadow-sm"
        >
          <RotateCcw class="w-5 h-5 text-sky-500" />
    ...
    <div v-if="result.answers && result.answers.length > 0" class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm">
      
      <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        <div>
          <h3 class="text-xl font-bold font-fredoka text-slate-800 flex items-center gap-2">
            <span>📋 Resumen de Respuestas</span>
          </h3>
          <p class="text-xs text-slate-500">Revisa tus respuestas para seguir aprendiendo</p>
        </div>

        <div class="flex items-center gap-3 text-xs font-bold font-fredoka">
          <span class="flex items-center gap-1 text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">
            ✓ {{ correctCount }} Correctas
          </span>
          <span v-if="wrongCount > 0" class="flex items-center gap-1 text-rose-700 bg-rose-50 px-3 py-1 rounded-full border border-rose-200">
      ...
      <div class="space-y-3.5">
        ...
        >
          ...
          <div class="flex items-center gap-4 w-full sm:w-auto">
            <div class="w-16 h-16 rounded-xl overflow-hidden bg-white flex-shrink-0 border border-slate-200 flex items-center justify-center shadow-xs">
              <img 
            ...
            <div>
              <div class="flex items-center gap-2">
                <span :class="['text-[11px] font-bold px-2 py-0.5 rounded-full font-fredoka', item.isCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700']">
                  {{ item.isCorrect ? 'Correcta ✓' : 'Incorrecta ✗' }}
                </span>
                <span class="text-xs text-slate-400">Pregunta {{ item.questionNumber }}</span>
              </div>
              <h4 class="text-lg font-bold font-fredoka text-slate-800 capitalize mt-0.5">
          ...
          <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100">
            <div v-if="!item.isCorrect" class="text-right text-xs">
              ...
              <span class="text-rose-600 font-semibold capitalize">{{ item.selectedOption?.englishWord || 'No respondida' }}</span>
            </div>
